  ...
  return (
    <div ref={containerRef} className={cn("scroller relative z-20 max-w-7xl overflow-hidden [mask-image:linear-gradient(to_right,transparent,white_20%,white_80%,transparent)]", className)}>
      <ul ref={scrollerRef} className={cn("flex w-max min-w-full shrink-0 flex-nowrap gap-4 py-4", start && "animate-scroll", pauseOnHover && "hover:[animation-play-state:paused]")}>
        {items.map((item, idx) => (
          <li
            className="relative w-[350px] max-w-full shrink-0 rounded-2xl border border-b-0 border-zinc-200 bg-[linear-gradient(180deg,#fafafa,#f5f5f5)] px-6 py-4 md:w-[350px] dark:border-zinc-700 dark:bg-[linear-gradient(180deg,#27272a,#18181b)]"
            key={idx}
          >
            <blockquote>
              ...
              <span className="relative z-20 text-3xl leading-[1.6] font-normal text-neutral-800 dark:text-gray-100">{item.amount}</span>
              <div className="relative z-20 mt-5 flex flex-row items-center">
                <span className="flex flex-col gap-1">
                  ...
                  <span className="text-sm leading-[1.6] font-normal text-neutral-500 dark:text-gray-400">{item.date}</span>
                </span>
              </div>
            </blockquote>
          </li>
